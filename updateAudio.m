function updateAudio(dur)


path = 'TMB_database\Noise Segments\';
names = dir(path);
...
for k = 1:length(folders)
    ...
    if folders(k) == 1
        ...
        for i = 1:n
            ...
            temp = audioread([path names(k).name '/' files(x(i)).name]);
            temp = temp(1:len);
        ...
        audiowrite([path names(k).name '.wav'], audio, fs); 
